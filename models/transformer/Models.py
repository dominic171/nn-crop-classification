...
from models.transformer.Layers import DecoderLayer, EncoderLayer
...
def get_attn_key_pad_mask(seq_k, seq_q):
    ...
    padding_mask = (
        padding_mask[:, :, 0].unsqueeze(1).expand(-1, len_q, -1)
    )  # b x lq x lk
...
class Encoder(nn.Module):
    ...
    def forward(self, src_seq, src_pos, return_attns=False):
        ...
        slf_attn_mask = torch.zeros(
            (src_seq.shape[0], src_seq.shape[1], src_seq.shape[1]), dtype=torch.uint8
        )
