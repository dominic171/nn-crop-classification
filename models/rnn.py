...
class RNN(ClassificationModel):
    def __init__(
        ...
        if use_layernorm:
            ...
            self.clayernorm = nn.LayerNorm(
                (hidden_dims + hidden_dims * bidirectional) * num_rnn_layers
            )
        ...
        if (
            bidirectional
        ):  # if bidirectional we have twice as many hidden dims after lstm encoding...
